  ...
  <div class="storeWarpper">
    <div class="header">
      <div class="logoWarrper">
        ...
        <img
          v-for="(item, index) in state.logoList"
          :key="index"
          :style="{ animationDelay: item.delay, left: item.left, right: item.right, bottom: item.bottom }"
          :class="['stars', 'l' + (index + 1), 'anima']"
          :src="item"
          alt=""
        />
      </div>
      ...
      <div class="storeList">
        <div class="item" v-for="(item, index) in state.storeList" :key="index">
          <div class="info">
            ...
            <span @click.stop="toPage(item.URL)" class="storeTitle">{{ item.storeName }}</span>
          </div>
          <div class="data">
            <nut-icon name="star" />
